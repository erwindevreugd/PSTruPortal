<#
    ...
    .EXAMPLE
    Invoke-DeactivateActionTrigger
    
    .LINK
    https://github.com/erwindevreugd/PSThruPortal
#>
function Invoke-DeactivateActionTrigger {
    [CmdletBinding()]
    param (
        [Parameter(
            Position=0, 
        ...
        [Parameter(
            Mandatory=$true,
            ValueFromPipelineByPropertyName=$true,
            HelpMessage="The id of the action trigger."
        )]
        [int]$ActionTriggerId
    )
    ...
    process {
        ...
        if($ActionTriggerId) {
            $uri = "$uri/$ActionTriggerId/execute?action=deactivation"
        }
